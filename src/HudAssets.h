...
inline HudAssets carregarHudAssets(SDL_Renderer *renderer)
{
    ...
    h.caixaPresente = carregarTextura(renderer, "assets/sprites/icons/hud/esquerdo/caixa_presente.png");
...
{
    if (h.painelStatus)
        SDL_DestroyTexture(h.painelStatus);
    if (h.iconeOuro)
        SDL_DestroyTexture(h.iconeOuro);
    if (h.iconeMoedaVerde)
        SDL_DestroyTexture(h.iconeMoedaVerde);
    if (h.iconeCoracao)
        SDL_DestroyTexture(h.iconeCoracao);
    if (h.iconeXp)
        SDL_DestroyTexture(h.iconeXp);
    if (h.avatarMasculino)
    ...
    if (h.caixaPresente)
        SDL_DestroyTexture(h.caixaPresente);
}
